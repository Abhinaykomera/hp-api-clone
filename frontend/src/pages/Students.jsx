...
const BLOOD_STATUSES = ['pure-blood', 'half-blood', 'muggle-born', 'squib', 'unknown'];

const BLOOD_COLORS = {
  'pure-blood':  'text-violet-300 bg-violet-900/40 border-violet-700/40',
  'half-blood':  'text-blue-300   bg-blue-900/40   border-blue-700/40',
  'muggle-born': 'text-emerald-300 bg-emerald-900/40 border-emerald-700/40',
  'squib':       'text-orange-300 bg-orange-900/40 border-orange-700/40',
  'unknown':     'text-slate-400  bg-slate-800/40  border-slate-600/40',
};

const YEAR_LABELS = { 1: '1st', 2: '2nd', 3: '3rd', 4: '4th', 5: '5th', 6: '6th', 7: '7th' };
...
function DeleteModal({ name, label, onClose, onConfirm }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div className="w-full max-w-sm rounded-2xl border border-red-500/30 bg-[#1a1a2e] shadow-2xl p-6 text-center">
        ...
        <h2 className="text-lg font-bold text-white mb-2">Delete {label}?</h2>
        <p className="text-sm text-slate-400 mb-5">
          Delete <span className="text-white font-semibold">{name}</span>? This cannot be undone.
        </p>
        {error && <p className="mb-4 text-sm text-red-400">{error}</p>}
        <div className="flex gap-3 justify-center">
          <button onClick={onClose} className="rounded-lg border border-[#2a2a4a] px-5 py-2 text-sm font-medium text-slate-300 hover:border-slate-500 transition-colors">Cancel</button>
          <button onClick={handle} disabled={loading} className="rounded-lg bg-red-600 hover:bg-red-500 px-5 py-2 text-sm font-semibold text-white disabled:opacity-50 transition-colors">
...
// ── Student form modal ──────────────────────────────────────────────
const BLANK = { name: '', house: '', year: '', bloodStatus: 'unknown' };

function StudentModal({ initial, houses, onClose, onSaved }) {
  const isEdit = Boolean(initial?._id);
  const [form, setForm]     = useState(() => initial
    ? { name: initial.name ?? '', house: initial.house?._id ?? initial.house ?? '', year: initial.year != null ? String(initial.year) : '', bloodStatus: initial.bloodStatus ?? 'unknown' }
    : BLANK);
  const [error, setError]   = useState('');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        bloodStatus: form.bloodStatus,
      };
      const { data } = isEdit
        ? await api.put(`/students/${initial._id}`, payload)
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div className="w-full max-w-lg rounded-2xl border border-[#2a2a4a] bg-[#1a1a2e] shadow-2xl">
        <div className="flex items-center justify-between border-b border-[#2a2a4a] px-6 py-4">
          <h2 className="text-lg font-bold text-white">{isEdit ? '✏️ Edit Student' : '📚 New Student'}</h2>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          <div>
            ...
            <input name="name" required value={form.name} onChange={set} placeholder="Hermione Granger" className={inputCls} />
          </div>
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select name="year" value={form.year} onChange={set} className={selectCls}>
                <option value="">— N/A —</option>
                {[1,2,3,4,5,6,7].map((y) => <option key={y} value={y}>{YEAR_LABELS[y]} Year</option>)}
              </select>
            </div>
          </div>
          <div>
            <label className={labelCls}>Blood Status</label>
            <select name="bloodStatus" value={form.bloodStatus} onChange={set} className={selectCls}>
              {BLOOD_STATUSES.map((s) => (
...
// ── Student card ────────────────────────────────────────────────────
function StudentCard({ student, token, onEdit, onDelete }) {
  const pal = getPalette(student.house?.name);
  const bloodCls = BLOOD_COLORS[student.bloodStatus] ?? BLOOD_COLORS.unknown;
  return (
    <div className={`relative rounded-2xl border ${pal} p-5 hover:scale-[1.01] transition-transform duration-200 shadow-md`}>
      <div className="flex items-start justify-between gap-2 mb-3">
        <div>
          ...
          <div className="flex flex-wrap items-center gap-1.5 mt-1">
            {student.house?.name && (
              <span className="text-[11px] text-slate-400 border border-[#2a2a4a] rounded-full px-2 py-0.5">{student.house.name}</span>
            )}
            {student.year && (
              <span className="text-[11px] text-[#d4af37]">{YEAR_LABELS[student.year]} Year</span>
            )}
          </div>
        </div>
        {token && (
      ...
      <div>
        <span className={`inline-block rounded-full border px-2.5 py-0.5 text-[11px] font-semibold ${bloodCls}`}>
          {student.bloodStatus}
        </span>
      </div>
    </div>
...
export default function Students() {
  ...
  const handleSaved   = (saved, isEdit) =>
    setStudents((p) => isEdit ? p.map((s) => s._id === saved._id ? saved : s) : [saved, ...p]);
  ...
  return (
    <>
      {modal?.mode === 'create' && (
        <StudentModal initial={null} houses={houses} onClose={() => setModal(null)} onSaved={handleSaved} />
      )}
      {modal?.mode === 'edit' && (
        <StudentModal initial={modal.item} houses={houses} onClose={() => setModal(null)} onSaved={handleSaved} />
      )}
      {modal?.mode === 'delete' && (
        <DeleteModal name={modal.item.name} label="Student" onClose={() => setModal(null)}
          onConfirm={async () => { await api.delete(`/students/${modal.item._id}`); handleDeleted(modal.item._id); }} />
      )}
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-slate-400 text-sm ml-14">
              {loading ? 'Loading…' : `${pagination?.total ?? students.length} student${(pagination?.total ?? students.length) !== 1 ? 's' : ''}`}
            </p>
          </div>
          {token && (
        ...
        {error && <ErrorBanner msg={error} onDismiss={() => { setError(''); fetchStudents(); }} />}
        {loading && <Spinner />}
        ...
        {!loading && !error && students.length === 0 && (
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <p className="text-slate-400 text-sm">
              {search || houseFil ? 'Try adjusting your filters.' : token ? 'Add the first student.' : 'Log in to add students.'}
            </p>
          </div>
        )}
        ...
        {!loading && students.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5">
            {students.map((s) => (
              <StudentCard key={s._id} student={s} token={token}
